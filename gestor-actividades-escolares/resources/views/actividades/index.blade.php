...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('actividades.pdf') }}" class="btn btn-danger">
            📄 Listado Activida/Alumnos
        </a>
        <a href="{{ route('actividades.create') }}" class="btn btn-success">➕ Añadir Actividad</a>
    </div>

    @if ($actividades->isEmpty())
        <div class="alert alert-info">No hay actividades resgistradas.</div>
    @else
                            ...
                            <td>
                                ...
                                <form action="{{ route('actividades.destroy', $actividad) }}" method="POST"
                                    ...
                                    <button type="submit" class="btn btn-sm btn-danger"
                                        onclick="return confirm('Are you sure you want to delete this actividad?')">Eliminar</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
